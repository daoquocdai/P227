...
class IO():
    def __init__(self, work_dir, save_log=True, print_log=True):
        ...
        self.split_timer = {}
        # Đổi pavi_logger thành tb_writer
        self.tb_writer = None
    ...
    def log(self, tag, value, step):
        """
        Thay thế log của Pavi bằng tensorboard log.
        Sử dụng: io.log('train/loss', loss_value, epoch)
        """
        try:
            if self.tb_writer is None:
                # Khởi tạo SummaryWriter vào thư mục làm việc
                self.tb_writer = SummaryWriter(log_dir=self.work_dir)

            # Ghi dữ liệu vào TensorBoard (mặc định ghi scalar)
            self.tb_writer.add_scalar(tag, value, step)
        except Exception as e:
            self.print_log(f"TensorBoard Log Error: {e}")
    ...
    def close_tb(self):
        """Đóng writer khi kết thúc chương trình"""
