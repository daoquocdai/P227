...
class VisionSession:
    ...
    def note_frame(self, frame_id: int):

        if self.last_processed_frame_id >= 0:
            gap = frame_id - self.last_processed_frame_id - 1
    ...
    def get_buffer(self, name: str, maxlen: int) -> deque:
        ...
        if not isinstance(value, deque):
            raise TypeError(f"{name} is not deque")
