"""Golden runner for the V2 standalone oracle versus V2VisionEngine.
...
NORMAL_VIDEO = PROJECT_ROOT / "tests" / "fixtures" / "vision_v2" / "demo.mp4"
...
def run_reference(
    ...
    *,
    ...
    # The standalone V2 oracle historically lived beside its model tree and relied on
    ...
    from legacy.vision_v2 import realtime

    trace = GoldenTrace("legacy-v2-oracle")
    cursor = SourceCursor()
    ...
    try:
        ...
        with ExitStack() as stack:
            stack.enter_context(patch.object(realtime, "BASE_DIR", str(VISION_ROOT)))
            ...
            stack.enter_context(patch.object(realtime.time, "time", lambda: cursor.timestamp))
...
def run_direct(
    ...
    *,
    ...
    trace = GoldenTrace("DIRECT-V2VisionEngine")
    cursor = SourceCursor()
    recorder = StageRecorder(trace, cursor, record_heavy_arrays=record_heavy_arrays)
    engine = V2VisionEngine(identity_enabled=False)
    engine.start()
...
class RecordingV2VisionEngine(V2VisionEngine):
...
def instrument_engine(engine: V2VisionEngine, trace: GoldenTrace, cursor: SourceCursor) -> None:
    engine.start()
    engine._ensure_initialized()
    assert engine._dependencies is not None
    dependencies = engine._dependencies
...
def run_local_hub(
    ...
    *,
    ...
    engine = RecordingV2VisionEngine(trace, cursor)
...
def main() -> None:
    ...
    reference = run_reference(args.video, args.frames, render_overlays=False)
    direct = run_direct(args.video, args.frames)
    report = {
        "video": str(args.video.resolve()),
        "frames": args.frames,
        "oracle": "legacy-v2-standalone",
