...
async def test_due_fall_calls_exactly_once_across_repeated_ticks():
    ...
    provider = FakeEmergencyCallProvider([CallResult(True, provider_reference="CA-primary")])
    ...
    assert provider.calls[0]["phone_number"] == contact["phone_e164"]
    with database_connection() as connection:
        attempt = connection.execute(
            """SELECT contact_id, provider_reference FROM emergency_escalation_attempts
               WHERE incident_id=? AND status='succeeded'""",
            (accepted.incident_id,),
        ).fetchone()
    assert (attempt["contact_id"], attempt["provider_reference"]) == (contact["id"], "CA-primary")


@pytest.mark.asyncio
async def test_concurrent_evaluations_claim_only_one_call():
    now = datetime.now(UTC)
    add_contact()
    ...
    results = await asyncio.gather(service.evaluate_once(), service.evaluate_once())
...
@pytest.mark.asyncio
async def test_inactive_highest_priority_is_skipped_for_first_active_contact():
    now = datetime.now(UTC)
...
@pytest.mark.asyncio
async def test_multiple_active_contacts_call_only_lowest_priority_contact():
    now = datetime.now(UTC)
    add_contact("+84900000003", priority=3, name="Secondary")
    primary = add_contact("+84900000001", priority=1, name="Primary")
    add_contact("+84900000002", priority=2, name="Backup")
    await create_fall(opened_at=now - timedelta(seconds=31))
    provider = FakeEmergencyCallProvider()

    await FallEscalationService(settings(), provider, now=lambda: now).evaluate_once()
    assert [call["phone_number"] for call in provider.calls] == [primary["phone_e164"]]


@pytest.mark.asyncio
async def test_primary_non_retryable_failure_never_falls_back():
    now = datetime.now(UTC)
    primary = add_contact("+84900000001", priority=1, name="Primary")
    add_contact("+84900000002", priority=2, name="Secondary")
    await create_fall(opened_at=now - timedelta(seconds=31))
    provider = FakeEmergencyCallProvider([CallResult(False, error_code="INVALID_NUMBER", retryable=False)])
    service = FallEscalationService(settings(), provider, now=lambda: now)
...
@pytest.mark.asyncio
async def test_primary_deactivated_between_retries_stops_without_fallback():
    clock = [datetime.now(UTC)]
    primary = add_contact("+84900000001", priority=1, name="Primary")
    add_contact("+84900000002", priority=2, name="Secondary")
    accepted = await create_fall(opened_at=clock[0] - timedelta(seconds=31))
    provider = FakeEmergencyCallProvider([CallResult(False, error_code="TIMEOUT", retryable=True)])
    service = FallEscalationService(settings(), provider, now=lambda: clock[0])

    await service.evaluate_once()
    emergency_contact_service.deactivate_contact(primary["id"])
    clock[0] += timedelta(seconds=11)
    await service.evaluate_once()
    ...
    with database_connection() as connection:
        attempt = connection.execute(
            """SELECT error_code, retryable FROM emergency_escalation_attempts
               WHERE incident_id=? AND contact_id=?""",
            (accepted.incident_id, primary["id"]),
        ).fetchone()
    assert (attempt["error_code"], attempt["retryable"]) == ("PRIMARY_CONTACT_INACTIVE", 0)
