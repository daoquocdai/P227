...
def get_tta_loader(config, tta_enabled=False):
    # Tự động tìm 'test_feeder', nếu không có thì lấy 'feeder'
...
def main():
    ...
    parser.add_argument('--grid_steps', type=int, default=20, 
                        help='Số bước nhảy Grid Search. 20 (bước 0.05), 10 (bước 0.1).')
    args = parser.parse_args()

    # Chỉ nhận 2 model đầu vào
    inputs = [
        (args.config1, args.weight1), 
        (args.config2, args.weight2)
    ]
    ...
    # ---------------------------------------------------------
    # [NEW] Trọng số độc lập cho từng mô hình, chạy từ 0 đến 1.0
    # ---------------------------------------------------------
    weights_range = np.linspace(0, 1.0, args.grid_steps + 1)
    # Lấy tất cả các tổ hợp trọng số độc lập (VD: (0.1, 0.5), (0.8, 1.0)...)
    combos = list(itertools.product(weights_range, repeat=num_models))
    
    # Quét tất cả các tổ hợp
    for current_weights in tqdm(combos, desc=f"Grid Search (Step {1/args.grid_steps:.2f})", ncols=100):
        # Bỏ qua trường hợp tất cả trọng số đều bằng 0
