...
export function QuickActionList({ disabled, status, onAction, exclude = [] }: { disabled: boolean; status?: AlertStatus; onAction: (action: QuickAction) => void; exclude?: QuickAction["id"][] }) { const terminal = status === "safe" || status === "false_alarm" || status === "resolved"; return <div className="quick-action-list">{quickActions.filter((action) => !exclude.includes(action.id)).map((action) => { const Icon = icons[action.id]; const actionDisabled = disabled || (terminal && ["safe", "help", "false_alarm"].includes(action.id)); return <button key={action.id} disabled={actionDisabled} className={`quick-${action.id}`} onClick={() => onAction(action)}><Icon /><span className="quick-label-desktop">{action.label}</span><span className="quick-label-mobile">{mobileLabels[action.id] ?? action.label}</span></button>; })}</div>; }
