...
export function AlertConversation({ alert, onBack, onStatus }: { alert: AlertEvent; onBack: () => void; onStatus: (status: AlertEvent["status"], note?: string) => void }) {
  ...
  const confirmSafe = () => { setSafeModal(false); onStatus("safe", "Người dùng xác nhận an toàn"); setMessages((items) => [...items, newMessage("assistant", "Cảm ơn bạn đã kiểm tra. Tôi đã đánh dấu sự kiện này là an toàn.", "success")]); };
  const falseAlarm = (reason: string, note: string) => { setFalseModal(false); onStatus("false_alarm", [reason, note].filter(Boolean).join(": ")); setMessages((items) => [...items, newMessage("assistant", "Đã ghi nhận phản hồi của bạn. Sự kiện được đánh dấu là cảnh báo sai.", "text")]); };
