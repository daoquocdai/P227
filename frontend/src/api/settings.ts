...
export interface SettingsUser { id:string; name:string; email:string; role:"admin"|"caregiver"; active:boolean; created_at:string; permissions:Record<PermissionKey,boolean>; temporary_password?:string; }
...
export const createSettingsUser = (data:{name:string;email:string;password:string;role:"caregiver"}) => apiClient<SettingsUser>("/settings/users",{method:"POST",body:JSON.stringify(data)});
