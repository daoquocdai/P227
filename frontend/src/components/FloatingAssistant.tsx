...
export default function FloatingAssistant() {
  ...
  const onPointerUp=(event:ReactPointerEvent<HTMLButtonElement>)=>{const drag=dragRef.current;if(drag.pointerId!==event.pointerId)return;if(event.currentTarget.hasPointerCapture(event.pointerId))event.currentTarget.releasePointerCapture(event.pointerId);skipClickRef.current=drag.moved;drag.pointerId=-1;setDragging(false);if(drag.moved)setPosition(current=>{if(current)localStorage.setItem("antam_pet_position",JSON.stringify(current));return current})};
  ...
  return <div style={position?{left:position.x,top:position.y,right:"auto",bottom:"auto"}:undefined} className={`floating-assistant companion-root ${open ? "is-open" : ""} ${urgentAlerts.length > 0 ? "has-urgent-alert" : ""} ${dragging?"is-dragging":""} ${openRight?"open-right":""} ${openBelow?"open-below":""}`}>
    ...
    <button className="assistant-pet" onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerUp} onPointerCancel={onPointerUp} onClick={() => {if(skipClickRef.current){skipClickRef.current=false;return}setPeekVisible(false);setOpen((value) => !value)}} aria-label={open ? "Đóng trợ lý An Tâm" : "Mở trợ lý An Tâm"} aria-expanded={open}>
