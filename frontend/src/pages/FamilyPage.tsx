...
export default function FamilyPage() {
  ...
  return <section className="family-page page-wrap">
    ...
    {visible.length ? <>
      <div className="family-table-wrap"><table className="family-table"><thead><tr><th>Ảnh</th><th>Tên</th><th>Mối quan hệ</th><th>Số ảnh khuôn mặt</th><th>Chất lượng nhận diện</th><th>Trạng thái</th><th>Hành động</th></tr></thead><tbody>
        {visible.map((person) => { const score = averageQuality(person); const quality = qualityInfo(score, person.faces.length); return <tr key={person.id} className={!person.active ? "hidden-person" : ""} onClick={() => setSelectedId(person.id)}>
          ...
          <td><div className="family-row-actions"><button className="family-profile-link" onClick={(event) => { event.stopPropagation(); setSelectedId(person.id); }}>Xem hồ sơ</button><button className="family-deactivate-control" onClick={(event) => { event.stopPropagation(); togglePerson(person); }}>{person.active ? "Vô hiệu hoá" : "Kích hoạt"}</button></div></td>
        </tr>; })}
      </tbody></table></div>
    ...
    {selected && <div className="family-modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && setSelectedId(null)}><article className="family-detail-modal">
      ...
      <section className={`person-active-setting family-deactivate-control ${!selected.active ? "warning" : ""}`}><div>{selected.active ? <ShieldCheck /> : <AlertTriangle />}<span><strong>Cho phép nhận diện người này</strong><small>{selected.active ? "Hệ thống đang coi đây là người quen." : "Người này có thể được đánh dấu là người lạ."}</small></span></div><button className={`family-switch ${selected.active ? "on" : ""}`} onClick={() => togglePerson(selected)}><i /></button></section>
